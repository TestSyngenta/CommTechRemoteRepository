  ...
  set<id> attids = new set<id>();
   ...
   if(Trigger.new<>null){
       for(ContentDocumentLink c:Trigger.new){
           Commercial_Experience_Demos__c	 l;
   ...
   if(attids.size()>0){
       try{
           ...
           Map<id,Commercial_Experience_Demos__c	> testmap = new Map<id,Commercial_Experience_Demos__c	>([select id,File_Count__c from Commercial_Experience_Demos__c	 where id IN: attids]);
